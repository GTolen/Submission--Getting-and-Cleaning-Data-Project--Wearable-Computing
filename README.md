...
# Other files were read and stored to data frames only to check contents and to see if there are information
# needed for producing output file which are not available from the X_train.txt and X_test.txt files.
...
# Use features data frame to specify the column names
...
# Sub-features will only include columns with -mean() or -std() in its names,
# subject_id and activity (#2)
